...
######################################################## 
  STREAM='http://centova6.ciclanohost.com.br:9518/stream'
  NRADIO='Arapuan FM - 107.3 FM'
  pkill mpv
  pkill mplayer
########################################################
${OPEN}
...
########################################################
${CONECTADO} | mplayer ${STREAM} -msglevel all=4 | 
yad --text-info --window-icon "audio-x-generic.png" --title "Rádio - ${NRADIO}" --center --height 300 --width 750 --tail --margins 4 --button="gtk-close"
fi
      break
####################################################################################
${CLOSE}