...
######################################################## 
  STREAM='https://cdn.painelstreaming.com.br:27092/stream'
  NRADIO='Antena Rock'
  pkill mpv
  pkill mplayer 
########################################################
...
########################################################
${CONECTADO} | mpv ${STREAM} | yad --text-info --window-icon "audio-x-generic.png" --title "Rádio - ${NRADIO}" --center --height 300 --width 750 --tail --margins 4 --button="gtk-close"
fi
      break
####################################################################################
${CLOSE}