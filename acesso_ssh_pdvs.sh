  ...
  do
    FAIXA_IP=
      FORMULARY=$(yad --center --width=380 \
          ...
          --title "ACESSO SSH (NPDVs)"                                                                   \
          --form                                                                                         \
          --field="DIGITE A FAIXA E O FINAL DO IP QUE DESEJA ACESSO SSH" $FAIXA_IP ""                    \
          --text="Acesso SSH por faixa e ip. Desenvolvido por: Nilsonlinux."                             \
...
################################################
${CONECTADO} | gnome-terminal -x bash -c "sshpass -p 1 ssh -oStrictHostKeyChecking=no root@${IPSERV}.${FAIXA_IP}; bash"
fi
      break
  done
#####################################################################################
