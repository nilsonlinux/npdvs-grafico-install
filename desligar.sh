  ...
  do
    IP=
      FORMULARY=$(yad --center --width=380 \
          --window-icon="gtk-execute" --image="$DIR/img/desligar.svg" --item-separator=","   \
          --title "DESLIGAMENTO DE PDVS (NPDVs)" \
          --form \
          --field="FAIXA E IP DO PDV QUE DESEJA DESLIGAR" $IP ""  \
          --text="Desligamento de PDVs. Desenvolvido por: Nilsonlinux."  \
...
( 
${CONECTADO}
echo "Desligando Terminal IP - ${IPSERV}.${IP}" ; sleep 1
datainicial=`date +%s`
sshpass -p 1 ssh -o "StrictHostKeyChecking no" root@${IPSERV}.${IP} "halt";
echo "Comando disparado para o IP - ${IPSERV}.${IP}. Por favor aguarde. ⌛️" ; sleep 1
echo "----------------------------------------------------------------" ; sleep 1
echo "Feito! Terminal com IP - ${IPSERV}.${IP} Desligado." ; sleep 1
echo "----------------------------------------------------------------" ; sleep 1
echo
echo "****************************************************************"
