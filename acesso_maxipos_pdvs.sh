  ...
  do
    FAIXA=
      FORMULARY=$(yad --center --width=480 \
          --window-icon="gtk-execute" --image="$DIR/img/vnc.svg" --item-separator=","            \
          --title "ACESSO MAXIPOS (WEBVIEW)"                                                              \
          --form                                                                                          \
          --field="DIGITE A FAIXA DA FILIAL E IP.  192.168." $FAIXA ""                                    \
          --text="Acesso vnc web por faixa e ip. Desenvolvido por: Nilsonlinux.
---------------------------------------------------------------------------------------------------
...
${AZUL}     C A I X A   05   ${END}                          106                                   192.168.3.${VERDE}159   ${END}
...
${CONECTADO} | xdg-open http://${IPSERV}.${FAIXA}.${PORTA_ACESSO}
fi
      break
  done
#####################################################################################
